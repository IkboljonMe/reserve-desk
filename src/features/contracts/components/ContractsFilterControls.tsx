...
export function ContractsFilterControls({ s, stack = false }: { s: ContractsPageState; stack?: boolean }) {
  const { t } = useTranslation()
  const {
    search, setSearch, statusFilter, setStatusFilter,
    expiryFilter, setExpiryFilter, sortKey, setSortKey, activeFilterCount,
  } = s

  return (
    <div className={stack ? 'flex flex-col gap-3' : 'flex items-center gap-3 flex-wrap'}>
      <div className={`flex items-center gap-2 ${stack ? 'w-full rounded-lg border border-[var(--surface-border)] px-3 py-2' : 'flex-1 min-w-[220px]'}`}>
      ...
      {activeFilterCount > 0 && (
        <Button
          ...
          onClick={() => { setStatusFilter(''); setExpiryFilter('all') }}
        >
          {t('clearFilters')}
        </Button>
      )}
