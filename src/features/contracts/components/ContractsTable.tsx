    ...
    <div className="bg-[var(--surface-card)] border border-[var(--surface-border)] rounded-[var(--radius-lg)] shadow-sm p-0 overflow-hidden max-[768px]:-mx-[1.1rem] max-[768px]:rounded-none max-[768px]:border-x-0">
      {loading ? (
        ...
        <EmptyState icon={
          ...
          {contracts.length === 0 && <Button className="mt-2" onClick={openAdd}>{t('addFirstContract')}</Button>}
        </EmptyState>
      ) : (
            ...
            <thead>
              <tr className="border-b border-[var(--gray-200,#e5e7eb)] bg-[var(--gray-50,#f9fafb)]">
                {([
                  ...
                  <th key={key} className="px-4 py-2.5 text-left font-semibold text-[var(--gray-500,#6b7280)] text-xs whitespace-nowrap">{label}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {visible.map((c, i) => {
                ...
                return (
                  <tr key={c._id} className={`border-b border-[var(--gray-100,#f3f4f6)] ${i % 2 === 0 ? 'bg-white' : 'bg-[var(--gray-50,#f9fafb)]'}`}>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="min-w-0">
                          <div className="font-semibold text-[var(--gray-800)] whitespace-nowrap overflow-hidden text-ellipsis max-w-[240px]">{c.organizationName}</div>
                          {c.inn && <div className="text-[0.72rem] text-[var(--gray-400)]">{t('inn')} {c.inn}</div>}
                        </div>
                      </div>
                    </td>
                    {multiHotel && (
                    ...
                    <td className="px-4 py-3 text-[var(--gray-700)] font-semibold whitespace-nowrap">{c.contractNumber || <span className="text-[var(--gray-300)]">—</span>}</td>
                    <td className="px-4 py-3 text-[var(--gray-600)]">
                      {c.representativeName ? (
                        <div className="min-w-0">
                          <div className="whitespace-nowrap">{c.representativeName}</div>
                          {c.phone && <div className="text-[0.72rem] text-[var(--gray-400)]">{c.phone}</div>}
                        </div>
                      ) : (c.phone || <span className="text-[var(--gray-300)]">—</span>)}
                    </td>
                    <td className="px-4 py-3">
                      <span
                      ...
                      >
                        <span className="w-1.75 h-1.75 rounded-full" style={{ background: sm.color }} />
                        {t(sm.labelKey)}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-[var(--gray-700)] whitespace-nowrap">{fmtDate(c.finishDate)}</td>
                    <td className="px-4 py-3"><ExpiryPill status={c.status} daysLeft={dl} /></td>
                    <td className="px-4 py-3">
                      {c.contractLink ? (
                        <a
                          href={c.contractLink}
                          ...
                          className="inline-flex items-center justify-center gap-1.5 rounded-sm py-1.5 px-3 text-[0.8125rem] font-semibold text-[var(--brand-600,#4f46e5)] whitespace-nowrap tracking-tight transition-colors duration-150 hover:bg-[var(--gray-50,#f9fafb)] hover:text-[var(--gray-800)]"
