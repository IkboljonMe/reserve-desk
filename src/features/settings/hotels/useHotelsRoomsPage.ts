...
export function useHotelsRoomsPage() {
  ...
  const [hotelDeleteConfirm, setHotelDeleteConfirm] = useState<string | null>(
    null,
  );
  ...
  const [roomDeleteConfirm, setRoomDeleteConfirm] = useState<string | null>(
    null,
  );
  ...
  function onShortNameChange(value: string) {
    ...
    setHotelForm((f) => ({
      ...f,
      shortName: value.toUpperCase().replace(/[^A-Z0-9]/g, ""),
    }));
  }
  ...
  const shortNameError = useMemo(() => {
    ...
    if (
      hotels.some(
        (h) => h._id !== editHotelId && h.shortName?.toUpperCase() === sn,
      )
    ) {
  ...
  async function handleSubmitHotel(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(
        editHotelId ? `/api/hotels/${editHotelId}` : "/api/hotels",
        {
  ...
  async function handleSubmitRoom(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(
        editRoomId ? `/api/rooms/${editRoomId}` : "/api/rooms",
        {
  ...
  function handleRoomDragOver(e: React.DragEvent, target: Room) {
    ...
    if (
      !dragged ||
      dragged.hotelId !== target.hotelId ||
      dragged.floor !== target.floor
    )
      return;
