      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-4">
        <div>
          <h2 className="m-0 text-lg font-bold text-[var(--gray-800)] flex items-center gap-2">
            <BedDouble size={18} className="text-[var(--brand-600)]" /> {t('rooms')}
          </h2>
      ...
      {loading ? null : rooms.length === 0 ? (
        <div className="bg-[var(--surface-card)] border border-[var(--surface-border)] rounded-[var(--radius-lg)] shadow-sm p-0 overflow-hidden">
          <EmptyState icon={<BedDouble size={26} />}>
            ...
            <p>{t('noRoomsDesc')}</p>
            <Button className="mt-2" onClick={openRoomModal}>{t('addFirstRoom')}</Button>
          </EmptyState>
        </div>
      ) : (
        <div className="flex flex-col gap-4">
          ...
          {unassignedRooms.length > 0 && (
            <div className="bg-[var(--surface-card)] border border-amber-300 rounded-[var(--radius-lg)] shadow-sm p-0 overflow-hidden">
              <div className="px-5 py-3 bg-amber-50 border-b border-amber-200 flex items-center gap-2">
                <TriangleAlert size={16} className="text-[var(--warning,#f59e0b)] shrink-0" />
                <div>
                  <div className="font-bold text-[0.8125rem] text-amber-800">
                    {t('unassignedRoomsCount', { count: unassignedRooms.length })}
                  </div>
                  <div className="text-[0.72rem] text-amber-700">{t('unassignedHint')}</div>
                </div>
              </div>
              {unassignedRooms.map(room => (
                <div key={room._id} className="flex items-center gap-2.5 px-5 py-3 border-b border-[var(--gray-100,#f3f4f6)] last:border-0">
                  <span className="font-bold text-[var(--gray-800)] text-[0.9375rem] min-w-[60px] tabular-nums">
                    #{room.number}
                  </span>
                  <span className="text-[0.72rem] text-[var(--gray-400)]">{t('floor')} {room.floor}</span>
                  ...
                  <Button variant="ghost" icon className="text-[var(--danger)]" onClick={() => handleDeleteRoom(room._id)} title={t('deleteRoomAria')} aria-label={t('deleteRoomAria')}>
                    <Trash2 size={15} />
                  </Button>
                </div>
              ))}
          ...
          {hotels.filter(h => (roomsByHotel.get(h._id) || []).length > 0).map(hotel => {
            ...
            return (
              <div key={hotel._id} className="bg-[var(--surface-card)] border border-[var(--surface-border)] rounded-[var(--radius-lg)] shadow-sm p-0 overflow-hidden">
                <div className="px-5 py-3 bg-[var(--gray-50,#f9fafb)] border-b border-[var(--gray-200,#e5e7eb)] flex items-center gap-2.5">
                  ...
                  <span className="font-semibold text-[0.8125rem] text-[var(--gray-700)]">{hotel.name}</span>
                      ...
                      {hotelRooms.filter(r => r.floor === floor).map(room => {
                        ...
                        >
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            ...
                            <span className="inline-flex items-center justify-center w-7.5 h-7.5 rounded-lg shrink-0 bg-[var(--brand-50,#eef2ff)] text-[var(--brand-600,#4f46e5)]">
                              <DoorClosed size={16} />
                            </span>
                            <div className="min-w-0 overflow-hidden">
                              <div className="font-bold text-[var(--gray-800)] text-[0.9375rem] whitespace-nowrap overflow-hidden text-ellipsis tabular-nums">
                                {displayCode(hotel)}-{room.number}
                                {room.type && (
                                  <span className="ml-2 text-[0.7rem] font-semibold text-[var(--brand-600,#4f46e5)] bg-[var(--brand-50,#eef2ff)] px-1.5 py-0.5 rounded-md inline-block align-middle">
                                    {room.type}
                                  </span>
                                )}
                              </div>
                          ...
                          {roomDeleteConfirm === room._id ? (
                            ...
                            <div className="flex gap-1 shrink-0">
                              ...
                              <Button variant="ghost" icon className="text-[var(--danger)]" onClick={() => setRoomDeleteConfirm(room._id)} title={t('deleteRoomAria')} aria-label={t('deleteRoomAria')}>
                                <Trash2 size={14} />
                              </Button>
                            </div>
                          )}
