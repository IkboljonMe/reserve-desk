...
export function useTelegramPage() {
  ...
  async function handleDisconnect() {
    ...
    try {
      ...
      showToast(
        err instanceof Error ? err.message : t("telegramDisconnectFailed"),
        "error",
      );
    } finally {
  ...
  async function handleSync() {
    ...
    try {
      ...
      showToast(
        err instanceof Error ? err.message : t("telegramSyncFailed"),
        "error",
      );
    } finally {
      setSyncing(false);
      ...
      (prev) =>
        prev && {
          ...
          topics: prev.topics.map((t) =>
            t.id === id ? { ...t, notificationsEnabled: enabled } : t,
          ),
        },
    );
    try {
      ...
      showToast(
        err instanceof Error ? err.message : t("telegramTopicUpdateFailed"),
        "error",
      );
      loadData();
    } finally {
