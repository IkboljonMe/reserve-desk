...
export function ServicesGrid({ s }: { s: ServicesPageState }) {
  ...
  const cardProps = (
    svc: Parameters<typeof openEditForm>[0],
    ...
    return (
      <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4">
        {filtered.map((svc) => (
          <ServiceCard
            key={svc._id}
            {...cardProps(svc, hotelMap.get(extractHotelId(svc.hotelId))?.name)}
          />
        ))}
      </div>
    );
  ...
  return (
    <>
      {hotels
        .filter((h) =>
          filtered.some((svc) => extractHotelId(svc.hotelId) === h._id),
        )
        .map((hotel) => {
          const hotelServices = filtered.filter(
            (svc) => extractHotelId(svc.hotelId) === hotel._id,
          );
          const unassigned = filtered.filter((svc) => {
            const hid = extractHotelId(svc.hotelId);
            return !hid || !hotelMap.has(hid);
          });
          return (
            <div key={hotel._id} className="mb-8">
              {/* Hotel group header */}
              <div className="flex items-center gap-2.5 mb-3.5">
                <span className="inline-flex items-center justify-center min-w-9.5 h-7 px-2 rounded-md bg-(--brand-500,#6366f1) text-white font-bold text-[0.75rem] tracking-wide shrink-0">
                  {hotel.shortName || hotel.name.slice(0, 2).toUpperCase()}
                </span>
                <span className="font-bold text-[0.9375rem] text-(--gray-700)">
                  {hotel.name}
                </span>
                <ChevronRight size={14} className="text-(--gray-300)" />
                <span className="text-[0.75rem] text-(--gray-400) tabular-nums">
                  {hotelServices.length}{" "}
                  {hotelServices.length === 1
                    ? t("serviceOne")
                    : t("servicesWord")}
                </span>
              </div>
              ...
              {hotel._id === hotels[hotels.length - 1]._id &&
                unassigned.length > 0 && (
                  <div className="mt-6">
                    <div className="flex items-center gap-2 mb-3.5">
                      <span className="font-bold text-[0.8125rem] text-(--warning,#f59e0b)">
                        {t("unassigned")}
                      </span>
                    </div>
                    <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4">
