...
export function ContactWidget(p: Props) {
  ...
  return (
    <div
      className="fixed right-5 bottom-5 z-1200 w-[min(340px,calc(100vw-32px))] bg-(--surface-card) border border-(--surface-border) shadow-[0_20px_50px_rgba(0,0,0,0.15)] p-[18px_18px_16px] animate-contact-in transition-colors duration-200"
      role="dialog"
      aria-label={p.title}
    >
      <button
        type="button"
        className="absolute top-2.5 right-2.5 w-7 h-7 border-none bg-(--gray-100) hover:bg-(--gray-200) text-(--gray-500) cursor-pointer inline-flex items-center justify-center transition-colors"
      ...
      {status === "success" ? (
        ...
        <form onSubmit={submit}>
          ...
          <input
            className="w-full box-border px-3 py-2.5 mb-2 border border-(--surface-border) bg-(--surface-bg) text-(--gray-900) placeholder:text-(--gray-400) text-sm outline-none focus:border-brand-500 focus:shadow-[0_0_0_3px_rgba(79,110,247,0.14)] transition-colors"
            value={name}
          ...
          <button
            className="w-full py-2.75 border-none bg-[linear-gradient(135deg,#4f6ef7,#3b5bdb)] text-white font-bold text-[0.9rem] cursor-pointer disabled:opacity-60 disabled:cursor-default"
            type="submit"
