...
export function Features({ t }: { t: Translate }) {
  return (
    <section id="features" className="max-w-[1200px] mx-auto px-5 lg:px-10 py-14">
      <h2 className="text-center font-extrabold tracking-[-0.02em] text-slate-900 mb-2.5 text-[clamp(1.5rem,3vw,2rem)]">{t('lpPillarsTitle')}</h2>
      <p className="text-center text-slate-500 max-w-155 mx-auto mb-10 text-base leading-relaxed">{t('lpPillarsSub')}</p>

      <div className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-5">
        {PILLARS.map(({ icon: Icon, title, desc, points }) => (
          <div key={title} className={`${CARD} bg-white border border-slate-200 p-7 transition-colors duration-200`}>
            <div className="w-11 h-11 rounded-xl mb-4 flex items-center justify-center bg-brand-50 text-brand-500">
              <Icon size={22} />
            </div>
            <h3 className="text-[1.1rem] font-bold mb-2 text-slate-900">{t(title)}</h3>
