...
export function useDashboardPage() {
  ...
  const services = useMemo(
    () => servicesRaw.filter((s) => s.isActive),
    [servicesRaw],
  );
  ...
  const [customFrom, setCustomFrom] = useState(
    format(subDays(nowUZ(), 29), "yyyy-MM-dd"),
  );
  ...
  const { data: bookingsRaw = [], isLoading: loading } = useBookingsQuery(
    range.from,
    range.to,
  );
  ...
  const bookings = useMemo(
    () => bookingsRaw.filter((b) => b.status !== "cancelled" && !b.masked),
    [bookingsRaw],
  );
  ...
  const rows = useMemo(() => {
    ...
    const filtered = bookings.filter((b) => {
      ...
      if (fPayment === "unpaid" && st !== "unpaid" && st !== "partial")
        return false;
      if (fPayment === "free" && st !== "free") return false;
      if (fType !== "all" && (b.bookingType || "custom") !== fType)
        return false;
      if (fState === "active" && b.finished) return false;
      if (fState === "finished" && !b.finished) return false;
      if (
        q &&
        !`${b.customerName} ${b.roomNumber} ${b.customerPhone}`
          .toLowerCase()
          .includes(q)
      )
        return false;
  ...
  function exportToExcel() {
    ...
    const maxLens = Object.keys(data[0] || {}).reduce(
  ...
  const analytics = useMemo(() => {
    ...
    const buckets = byWeek
      ? eachWeekOfInterval({ start: from, end: to }, { weekStartsOn: 1 }).map(
          (s) => ({ start: s, end: addDays(s, 6) }),
        )
    ...
    const data = buckets.map((bk) => {
      const s = format(bk.start, "yyyy-MM-dd"),
        e = format(bk.end, "yyyy-MM-dd");
      const inBucket = bookings.filter((b) => b.date >= s && b.date <= e);
      const expected = inBucket.reduce(
        (tot, b) => tot + (b.totalPrice || 0),
        0,
      );
      const collected = inBucket.reduce(
        (tot, b) => tot + amountCollected(b),
        0,
      );
      return {
        label: byWeek
          ? format(bk.start, "MMM d", { locale })
          : format(bk.start, span <= 8 ? "EEE d" : "MMM d", { locale }),
        expected,
        collected,
  ...
  const perService = useMemo(() => {
    ...
    bookings.forEach((b) =>
      map.set(svcId(b), (map.get(svcId(b)) || 0) + (b.totalPrice || 0)),
    );
    return services
  ...
  const occupancy = useMemo(() => {
    ...
    bookings.forEach((b) => {
      bookedByService.set(
        svcId(b),
        (bookedByService.get(svcId(b)) || 0) + (b.duration || 0),
      );
      if (b.date) dowCount[weekdayOf(b.date)]++;
    });
    const perSvc = services
      .map((s) => {
    ...
    const peakDow = dowCount.some((c) => c > 0)
      ? dowCount.indexOf(Math.max(...dowCount))
      : null;
