...
export default function NotificationsPage() {
  ...
  const { items, loading, refetch, dismissing, dismiss, grouped } =
      ...
      {loading ? (
        <div className="flex flex-col gap-3">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="bg-(--surface-card) border border-(--surface-border) rounded-(--radius-lg) shadow-sm p-4 flex items-center gap-3"
            >
