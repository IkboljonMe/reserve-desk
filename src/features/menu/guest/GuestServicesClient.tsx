...
import { localized, guestHubPath, MENU_LANGS, MENU_LANG_LABELS } from '@/lib/menu'
...
export function GuestServicesClient({
  ...
  const [selectedService, setSelectedService] = useState<GuestServiceDto | null>(null)
  ...
  async function submitRequest() {
    ...
    if (!roomNumber.trim()) { setError(labels.errorRoomRequired); return }
  ...
  return (
    <div className="min-h-dvh bg-[var(--surface-bg)] text-[var(--gray-900)] pb-12" style={themeVars}>
      ...
      <main className="max-w-[448px] mx-auto px-4 py-5 flex flex-col gap-5">
        {services.length === 0 ? (
          <p className="text-center text-[var(--gray-400)] text-sm py-16">{labels.noServices}</p>
        ) : (
          <div className="grid grid-cols-1 gap-4">
            {services.map(s => {
              ...
              return (
                <div key={s._id} className="bg-[var(--surface-card)] border border-[var(--surface-border)] rounded-2xl overflow-hidden shadow-sm flex flex-col">
                  {s.imageUrl && (
                    <img src={s.imageUrl} alt="" className="w-full h-44 object-cover" />
                  )}
                  <div className="p-4 flex flex-col gap-2">
                    <h3 className="font-extrabold text-[1.1rem] text-[var(--gray-800)] m-0">{name}</h3>
                    {desc && <p className="text-[0.85rem] text-[var(--gray-500)] m-0 leading-snug">{desc}</p>}
                    <div className="mt-2 flex items-center justify-between">
                      <div className="font-extrabold text-[1.05rem] text-[var(--brand-600)] tabular-nums">
                        {s.price > 0 ? `${money(s.price)} ${labels.sum}` : ''}
                      </div>
                      <Button size="sm" onClick={() => setSelectedService(s)} leftIcon={<Sparkles size={14} strokeWidth={2.5} />}>
                        {labels.bookNow}
                      </Button>
                    </div>
                  </div>
      ...
      <Modal
        ...
        footer={success ? (
          <Button variant="secondary" className="w-full justify-center" onClick={closeModal}>{labels.close}</Button>
        ) : (
          <Button className="w-full justify-center" loading={submitting} onClick={submitRequest}>
      ...
      >
        {success ? (
          <div className="py-6 text-center">
            ...
            <p className="text-[1.1rem] font-bold text-[var(--gray-800)] mb-2 m-0">{labels.requestSent}</p>
            <p className="text-sm text-[var(--gray-500)] m-0">{labels.requestSentDesc}</p>
          </div>
        ) : selectedService ? (
          <div className="flex flex-col gap-4">
            <div className="rounded-xl border border-[var(--surface-border)] bg-[var(--surface-bg)] p-3">
              <div className="font-bold text-[0.95rem] text-[var(--gray-800)]">
                {localized(selectedService.nameI18n, selectedService.name, contentLang)}
              </div>
            ...
            <div className="flex flex-col gap-2.5">
              <input className={FIELD} value={roomNumber} onChange={e => setRoomNumber(e.target.value)} placeholder={labels.roomNumber} />
              <input className={FIELD} value={guestName} onChange={e => setGuestName(e.target.value)} placeholder={labels.guestNamePlaceholder} />
              <textarea className={`${FIELD} resize-y min-h-[70px]`} value={note} onChange={e => setNote(e.target.value)} placeholder={labels.orderNotePlaceholder} />
