...
async function fetchSettings(
  hotelId: string,
): Promise<Partial<Omit<MenuSettingsForm, "tiles"> & { tiles: TileConfig[] }>> {
  const r = await fetch(
    `/api/menu/settings?hotelId=${encodeURIComponent(hotelId)}`,
  );
...
async function saveSettings(
  data: { hotelId: string } & Partial<
    Omit<MenuSettingsForm, "tiles"> & { tiles: TileConfig[] }
  >,
) {
...
export function useMenuSettings(hotelId: string) {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      serviceFeeType:
        (d.serviceFeeType as MenuSettingsForm["serviceFeeType"]) ?? "none",
  ...
  const mutation = useMutation({
    mutationFn: (data: typeof form) => {
      const tiles: TileConfig[] = data.tiles.map((tile, i) => ({
        ...
        sortOrder: i,
        labelUz:
          tile.label.uz !== TILE_META[tile.id].label.uz ? tile.label.uz : "",
        labelRu:
          tile.label.ru !== TILE_META[tile.id].label.ru ? tile.label.ru : "",
        labelEn:
          tile.label.en !== TILE_META[tile.id].label.en ? tile.label.en : "",
      }));
  ...
  const setField = <K extends keyof MenuSettingsForm>(
    key: K,
    value: MenuSettingsForm[K],
