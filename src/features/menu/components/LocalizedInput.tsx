...
export function LocalizedInput({
  label, value, onChange, sourceLang, locked, onLockedChange, onTranslate, textarea, placeholder,
}: {
  label: string
  value: LocalizedText
  ...
  onTranslate: (text: string, sourceLang: string, skip: string[]) => Promise<Record<string, string>>
  textarea?: boolean
  placeholder?: string
}) {
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div className="flex flex-wrap gap-1">
        {MENU_LANGS.map(lang => (
          ...
          >
            ...
            {isLocked(lang) && <Lock size={9} className="inline ml-1 -mt-0.5" />}
          </button>
        ))}
      </div>
      ...
      {activeLang !== sourceLang && (
        <label className="inline-flex items-center gap-1.5 cursor-pointer select-none">
          ...
          />
          <span className="text-[0.72rem] text-[var(--gray-500)] font-medium">{t('keepOriginalLang')}</span>
        </label>
      )}
    </div>
  )
