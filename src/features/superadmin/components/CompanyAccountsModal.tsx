...
export function CompanyAccountsModal({
  company,
  onClose,
}: {
  ...
  async function confirmReset(id: string) {
    ...
    try {
      ...
      showToast(
        err instanceof Error ? err.message : t("passwordResetFailed"),
        "error",
      );
    } finally {
      setSavingId(null);
  ...
  return (
    <Modal
      open={!!company}
      onClose={onClose}
      title={`${company.name} — ${t("loginDetails")}`}
      size="lg"
    >
      <div className="mb-3 text-[0.8125rem] text-gray-500">
        {isExpired(company.expiresAt) ? (
          ...
          <>
            {t("planActiveUntil")}{" "}
            <strong>{new Date(company.expiresAt).toLocaleDateString()}</strong>
          </>
        )}
      </div>
