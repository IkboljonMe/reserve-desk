...
export function useCalendarPage() {
  ...
  const visibleBookings = useMemo(() => {
    ...
    return bookings.filter((b) => {
      ...
      if (hid && knownHotelIds.has(hid) && !selectedHotels.has(hid))
        return false;
      if (statusFilter !== "all") {
        const st = bookingState(b).key;
        if (statusFilter === "unpaid" && st !== "unpaid" && st !== "partial")
          return false;
        if (statusFilter === "paid" && !(st === "paid" || st === "free"))
          return false;
        if (statusFilter === "finished" && st !== "finished") return false;
      }
      if (q) {
        const hay =
          `${b.customerName} ${b.roomNumber} ${b.customerPhone}`.toLowerCase();
  ...
  async function updateBooking(
    ...
    try {
      ...
      setSelectedBooking((prev) =>
        prev && prev._id === id ? { ...prev, ...changes } : prev,
      );
  ...
  const markPaid = (b: Booking) =>
    updateBooking(
      b._id,
      { paid: true, amountPaid: b.totalPrice || 0 },
      t("markedAsPaid"),
    );
  ...
  const markFinished = (b: Booking) =>
    updateBooking(b._id, { finished: true }, t("bookingCompleted"));
  ...
  async function saveBookingEdit(id: string, changes: Partial<Booking>) {
    try {
      ...
      showToast(
        e instanceof Error && e.message ? e.message : t("updateFailed"),
        "error",
      );
    }
  }
  ...
  const allHotelsSelected =
    hotels.length > 0 && selectedHotels.size === hotels.length;
