...
export function HotelFilterList({ s }: { s: CalendarPageState }) {
  ...
  const {
    hotels, selectedHotels, setSelectedHotels, allHotelsSelected, visibleBookings, serviceHotel,
  } = s
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2.25 text-left p-[5px_7px] rounded-lg border-none cursor-pointer transition-all duration-120 font-sans ${
                checked ? 'bg-brand-500/10 opacity-100' : 'bg-transparent opacity-50'
              }`}
            >
...
export function ServiceFilterList({ s }: { s: CalendarPageState }) {
  const { t } = useTranslation()
  const {
    services, selectedServices, setSelectedServices, allSelected, visibleBookings,
  } = s
