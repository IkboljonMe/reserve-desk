      ...
      footer={
        <div className="flex flex-col gap-3.5">
          {selectedBooking.finished ? (
            <div className="flex items-center justify-center gap-2 p-2.5 rounded-lg bg-emerald-500/10 text-emerald-600 font-bold text-[0.85rem]">
              <Check size={16} /> {t("completed")}
            </div>
          ) : bookingState(selectedBooking).key === "unpaid" ||
            bookingState(selectedBooking).key === "partial" ? (
            <Button
              className="w-full"
              onClick={() => setPayConfirm(selectedBooking)}
            >
              <Wallet size={15} />{" "}
    ...
    >
      <div className="flex flex-col gap-3.5 text-sm">
        ...
        <DetailRow
          ...
          label={t("date")}
          value={format(parseISO(selectedBooking.date), "EEEE, MMM d, yyyy")}
        />
        <DetailRow
          icon={<Clock size={15} />}
          label={t("time")}
        ...
        {selectedBooking.totalPrice > 0 && (
          <DetailRow
            icon={<span className="font-bold text-[11px]">{t("sum")}</span>}
            label={t("price")}
            value={`${money(selectedBooking.totalPrice)} ${t("sum")}`}
            success
          />
        )}
        <DetailRow
          ...
          <DetailRow
            ...
            value={
              selectedBooking.menuItems
                ...
                .join(", ") +
              (selectedBooking.menuReadyTime
                ? ` · ${t("menuReadyTime")} ${selectedBooking.menuReadyTime}`
                : "")
            }
          />
        )}
