    ...
    <div className="p-3.5">
      <div className="grid grid-cols-7 gap-0.75 mb-1">
        {dayNames.map((d) => (
          <div
            key={d}
            className="text-center text-[0.7rem] font-bold text-(--gray-400) py-1 uppercase tracking-wider"
          >
            {d}
          </div>
        ))}
        ...
        {days.map((day) => {
          ...
          return (
            <div
              ...
              className={`min-h-26 p-1.5 transition-all duration-120 flex flex-col gap-0.75 border ${
                isPastDay
                  ? "bg-(--gray-50) border-(--gray-200) opacity-60 cursor-not-allowed"
                  : `cursor-pointer hover:shadow-sm hover:border-(--brand-400) ${
                      isToday
            ...
            >
              <div
                className={`text-[0.75rem] w-5.5 h-5.5 flex items-center justify-center rounded-full self-start ${
                  isToday
              ...
              {list.slice(0, 3).map((b) => {
                ...
                if (b.masked) {
                  return (
                    <div
                      ...
                      className="flex items-center gap-1.25 border-l-[3px] border-l-(--gray-400) p-[2px_5px_2px_6px] text-[0.68rem] text-(--gray-500) overflow-hidden whitespace-nowrap text-ellipsis cursor-default"
                      style={{
                        background:
                          "repeating-linear-gradient(45deg, var(--gray-100), var(--gray-100) 5px, var(--gray-200) 5px, var(--gray-200) 10px)",
                      }}
                    >
                ...
                return (
                  <div
                    ...
                    className="flex items-center gap-1.25 p-[2px_5px_2px_6px] text-[0.68rem] text-(--gray-700) overflow-hidden whitespace-nowrap text-ellipsis border-l-[3px] cursor-pointer"
                    style={{
              ...
              {list.length > 3 && (
                <div className="text-[0.68rem] text-(--gray-400) pl-1 font-semibold">
                  +{list.length - 3} more
                </div>
              )}
            </div>
