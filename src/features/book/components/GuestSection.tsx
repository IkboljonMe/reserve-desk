  ...
  return (
    <div>
      ...
      {bookingType === "client" && (
        <div className="form-group mb-5">
          <label className="form-label">
            {t("guestsIn", { group: categoryMeta?.label ?? "" })}
          </label>

          {selectedClientId ? (
            <div className="flex items-center gap-2.5 p-[10px_12px] rounded-lg border-[1.5px] border-brand-500 bg-brand-50">
              <div className="w-7.5 h-7.5 rounded-full bg-brand-100 text-brand-600 flex items-center justify-center font-bold text-[0.8rem] shrink-0">
            ...
            <>
              ...
              {clientResults.length > 0 ? (
                <div className="flex flex-col gap-1 max-h-42 overflow-y-auto">
                  {clientResults.map((c) => (
                    <button
                      key={c._id}
                      type="button"
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-[0.75rem] text-gray-400 flex items-center gap-0.75">
                          {c.roomNumber && (
                ...
                <p className="text-[0.8rem] text-gray-400 m-0 mb-2.5">
                  {clientSearch
                    ? t("noClientFoundName", { name: clientSearch.trim() })
                    : t("noSavedGuests")}
                </p>
              )}
              <Button
                ...
                className={`inline-flex items-center gap-1.5 ${clientResults.length > 0 ? "mt-2" : ""}`}
              >
      ...
      {bookingType === "room" && (
        <div className="form-group mb-5">
          <label className="form-label">
            {t("pickARoom")} ({categoryMeta?.label})
          </label>
