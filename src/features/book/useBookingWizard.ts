...
export function useBookingWizard({
  ...
  const [date, setDate] = useState(
    initialDate || nowUZ().toISOString().split("T")[0],
  );
  ...
  const { data: rawServices = [], isLoading: loadingServices } = useQuery({
    queryKey: ["services"],
    queryFn: getServices,
  });
  const { data: rms = [], isLoading: loadingRooms } = useQuery({
    queryKey: ["rooms"],
    queryFn: () => fetch("/api/rooms").then((r) => r.json()),
  });
  const { data: htls = [], isLoading: loadingHotels } = useQuery({
    queryKey: ["hotels"],
    queryFn: getHotels,
  });
  const { data: grps = [], isLoading: loadingGroups } = useQuery({
    queryKey: ["client-groups"],
    queryFn: () => fetch("/api/client-groups").then((r) => r.json()),
  });

  const services = useMemo(
    () =>
      Array.isArray(rawServices)
        ? rawServices.filter((s: Service) => s.isActive)
        : [],
    [rawServices],
  );
  ...
  const initialLoading =
    loadingServices || loadingRooms || loadingHotels || loadingGroups;
  ...
  useEffect(() => {
    ...
    getBookings(date, date).then((data) => {
      type BookingRow = DayBooking & { serviceId: string | { _id: string } };
      const filtered = Array.isArray(data)
        ? (data as BookingRow[]).filter(
            (b) =>
              b.status !== "cancelled" &&
              (typeof b.serviceId === "string"
                ? b.serviceId
                : b.serviceId?._id) === selectedService._id,
          )
        : [];
  ...
  function resolveGroupMeta(g: PricingGroup): { label: string; color: string } {
    if (g.target === "client") {
      ...
      return {
        label: cg?.name ?? t("unknownGroup"),
        color: cg?.color ?? "var(--gray-500)",
      };
    }
    return {
      label: g.category,
      color: selectedService?.color ?? "var(--brand-500)",
    };
  }
  ...
  const pricingSource: { pricingGroups?: PricingGroup[] } | null = hasVariants
    ? selectedVariant
    : selectedService;

  // Category groups available for the current pricing source, filtered to still-valid ones.
  const clientCats = (pricingSource?.pricingGroups ?? []).filter(
    (g) =>
      g.target === "client" &&
      g.rows.length > 0 &&
      clientGroups.some((c) => c._id === g.category),
  );
  const roomCats = (pricingSource?.pricingGroups ?? []).filter(
    (g) => g.target === "room" && g.rows.length > 0,
  );

  // "Ungrouped" (shown to the guest as "Custom") clients have no configured
  // pricing group, so duration/price are entered manually.
  const isUngroupedClient =
    bookingType === "client" && selectedCategory === UNGROUPED;
  ...
  const ratePerHour = selectedRate
    ? Math.round(selectedRate.price / Math.max(1, selectedRate.duration / 60))
    : 0;
  ...
  const activePlan: PricingPlan | null = usingManualPrice
    ...
    : selectedRate
      ? wholeDay
        ? {
            duration: dayMinutes,
            price: Math.round(ratePerHour * (dayMinutes / 60)),
          }
  ...
  const categoryMeta = isUngroupedClient
    ...
    : activeGroup
      ? resolveGroupMeta(activeGroup)
      : null;

  // Rooms of the chosen category in the selected hotel (room booking type)
  const categoryRooms = rooms.filter(
    (r) => r.hotelId === selectedHotelId && (r.type || "") === selectedCategory,
  );

  const roomLabel = (r: Room) =>
    `${hotels.find((h) => h._id === r.hotelId)?.shortName || "??"}-${r.number}`;

  const timeSlots =
    selectedService && activePlan && dayHours && !closedOnDate
  ...
  const availableSlots = activePlan
    ? timeSlots.filter((slot) => {
        ...
        const overlaps = dayBookings.filter(
          (b) =>
            toMin(b.startTime) < end + bufAfter &&
            toMin(b.endTime) > start - bufBefore,
        ).length;
  ...
  const guestReady =
    bookingType === "client" ? !!customerName.trim() : !!selectedRoomId;
  ...
  const slides = useMemo<SlideKey[]>(
    () =>
      SLIDE_KEYS.filter(
        (k) => k !== "hotel" || initialLoading || hotels.length > 1,
      ),
  ...
  async function confirmBooking() {
    ...
    const finalName =
      customerName.trim() || (bookingType === "room" ? roomNumber : t("guest"));
