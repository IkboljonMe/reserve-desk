...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await connectDB()
    const product = await MenuProduct.findOneAndUpdate(idScope(session, id), update, { new: true }).lean()
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params
  await connectDB()
  const deleted = await MenuProduct.findOneAndDelete(idScope(session, id)).lean()
