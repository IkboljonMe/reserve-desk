...
export default async function DemoHubPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const menuHotel = menuSettings
    ...
    : null
  const menuHref = menuHotel?.slug ? guestHubPath(locale, menuHotel.slug, '101') : null

  return (
    <main className="min-h-[100dvh] flex items-center justify-center bg-[radial-gradient(900px_500px_at_80%_-10%,rgba(124,58,237,0.35),transparent_60%),radial-gradient(800px_500px_at_10%_110%,rgba(79,110,247,0.30),transparent_55%),linear-gradient(135deg,#14192a_0%,#1e2540_50%,#14192a_100%)] px-4 py-10">
      <div className="w-full max-w-[720px]">
        <div className="text-center mb-10">
          <BrandMark size={64} priority className="mx-auto mb-3" />
          <h1 className="text-white text-[1.6rem] font-extrabold tracking-tight mb-1">{t('demoHubTitle')}</h1>
...
function DemoCard({ href, icon, title, desc, cta }: { href: string | null; icon: React.ReactNode; title: string; desc: string; cta: string }) {
  if (!href) return null
  return (
    <a
      href={href}
      className="group flex flex-col gap-3 bg-white/6 backdrop-blur-2xl border border-white/12 rounded-[20px] p-6 shadow-[0_24px_60px_rgba(0,0,0,0.45)] no-underline transition-transform hover:-translate-y-1"
    >
      <span className="w-12 h-12 rounded-2xl bg-[image:var(--brand-gradient)] text-white flex items-center justify-center">
        {icon}
      </span>
