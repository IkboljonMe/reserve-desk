  ...
  return (
    <div className="h-[100dvh] relative">
      ...
      <div className="flex h-full overflow-hidden relative">
        ...
        <div className="absolute top-0 left-0 right-0 h-[3px] bg-[var(--brand-gradient)] z-[2]" />
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0">
          <div className="hidden max-md:flex items-center gap-3 p-[0.85rem_1rem] border-b border-[var(--surface-border)] bg-[var(--surface-card)]">
            <button
              type="button"
              onClick={() => setMobileNavOpen(true)}
              aria-label={t('openMenuAria')}
              className="w-[38px] h-[38px] shrink-0 inline-flex items-center justify-center rounded-lg border border-[var(--gray-200)] bg-white text-[var(--gray-700)] cursor-pointer"
            >
          ...
          <main className={`flex-1 overflow-auto bg-[var(--surface-card)] ${isMobile ? 'p-[1.1rem]' : 'py-[1.75rem] px-8'}`}>
            {children}
          </main>
        </div>
      </div>
