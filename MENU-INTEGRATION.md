...
- ✅ **Guest page delivery:** `https://app.bronit.uz/en/menu?hotel=safirhotel&room=101` — served on the shared **app** subdomain (no company-specific subdomain needed). `?hotel=` = hotel (`Hotel.slug`, globally unique), `?room=` = room number, `/en` = locale. The hotel slug resolves both hotel and company server-side. No login required — the page is fully public, mobile-only layout.
